...
def assign_segments_to_cells(fishnet_gdf, features_gdf):
    ...
    # Create a spatial index for features
    sindex = features_gdf.sindex
    ...
    def process_cell(cell):
        ...
        if not precise_matches.empty:
            features_in_cell = gpd.clip(precise_matches, cell.geometry)
            total_length = features_in_cell.geometry.length.sum()
        else:
            total_length = 0
        return total_length

    # Using Dask to parallelize the operation
    lengths = []
    for idx, cell in fishnet_gdf.iterrows():
        length = delayed(process_cell)(cell)
        lengths.append(length)

    feature_lengths = dask.compute(*lengths)
...
def convert_length_to_density(fishnet_gdf, crs):
    ...
    if crs.axis_info[0].unit_name == 'metre':
        ...
        fishnet_gdf['density'] = fishnet_gdf['length_km'].astype(np.float32)
    else:
...
def process_tile(tile_key, feature_type, run_mode='default'):
    ...
    chunk_size = 2  # 2x2 degree chunks
