...
def create_output_dir_name_list(dir_list, start_year, chunk_size_pixels,
                                model_type, model_version, model_path_description, output_years,
...
def calculate_stats(array_per_ha, name, bounds_str, tile_id, in_out, array_per_pixel=None, nodata_val=np.nan):
    ...
    if in_out == 'output_layer' and array_per_pixel is not None:
        sum_value = float(np.nansum(array_per_pixel))  # Need nansum because SOC timeseries uses NaN
    else:
