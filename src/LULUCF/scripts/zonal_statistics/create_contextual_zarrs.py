...
This needs to use zarr v3.1.6, the zarr version that everything except flox zonal stats uses.
Thus, the cluster name can't include zonal_stats in it, or create_cluster.py will create a cluster that uses zarr v3.1.3,
the version needed for flox zonal stats.

python -m src.utilities.create_cluster -n 50 -m 64 -cn zarr_creation
python -m src.LULUCF.scripts.zonal_statistics.create_contextual_zarrs -cn zarr_creation -l X -ln "X zarr creation for zonal stats"
