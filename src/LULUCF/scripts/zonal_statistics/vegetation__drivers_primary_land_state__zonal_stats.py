"""
Zonal stats for vegetation using just land state, initial composite primary forest, and TCL 1km driver.
...
Testing (https://app.asana.com/1/25496124013636/task/1211023128606896/comment/1214642931490435?focus=true)
shows that the load for this script is small enough to use 32GB workers on 10x10 deg tiles.

Run from /mnt/c/GIS/git/AFOLU_GHG_flux_model

Coiled small tests:
python -m src.utilities.create_cluster -n 1 -m 32 -cn vegetation_zonal_stats --zonal_stats
python -m src.LULUCF.scripts.zonal_statistics.vegetation__drivers_primary_land_state__zonal_stats -cn vegetation_zonal_stats -bb 10 49 11 50 -fv 2 -ft 2 -mt standard -mpd global --input_date YYYYMMDD -zd test_box

Coiled 8-tile test (Central and East Africa):
python -m src.utilities.create_cluster -n 50 -m 32 -cn vegetation_zonal_stats --zonal_stats
python -m src.LULUCF.scripts.zonal_statistics.vegetation__drivers_primary_land_state__zonal_stats -cn vegetation_zonal_stats -bb 13 -14 44 -3 -fv 3 -ft 3 -mt standard -mpd global --input_date YYYYMMDD -zd Central_Africa_test

Coiled Cerrado test (174 features):
python -m src.utilities.create_cluster -n 50 -m 32 -cn vegetation_zonal_stats --zonal_stats
python -m src.LULUCF.scripts.zonal_statistics.vegetation__drivers_primary_land_state__zonal_stats -cn vegetation_zonal_stats -mt standard -mpd global --input_date YYYYMMDD -zd Cerrado_test -cshp s3://gfw2-data/climate/AFOLU_flux_model/fishnet_1x1deg/20250429/fishnet_GADM41_1x1deg__spatial_join_intersect__20250428__center_in__Cerrado_center_in.shp
...
Full run:
python -m src.utilities.create_cluster -n 50 -m 32 -cn vegetation_zonal_stats --zonal_stats
python -m src.LULUCF.scripts.zonal_statistics.vegetation__drivers_primary_land_state__zonal_stats -cn vegetation_zonal_stats -mt standard -mpd global --input_date YYYYMMDD -zd global -cshp s3://gfw2-data/climate/AFOLU_flux_model/fishnet_1x1deg/20250429/fishnet_GADM41_1x1deg__spatial_join_intersect__20250428__center_in.shp --log_note  -ln "Global zonal stats for vegetation model v1.0.5 against TCL driver, starting composite primary forest, and land state only."
"""
